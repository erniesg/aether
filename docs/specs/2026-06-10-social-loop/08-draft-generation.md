...
- Depends: hard: 04 (accepted strategy is the input), 05 (queue is the output); soft: 03/06/07 (facts, ledger, insights enrich the prompt when present)
- Evidence dir: docs/specs/2026-06-10-social-loop/evidence/08/

## Summary

The brain of the presence loop: one action turns the accepted strategy into pillar-tagged post drafts and reply candidates sitting in the publish queue — every draft grounded in a receipt (a repo fact or a real ICP thread). Nothing auto-posts; the queue's edit/confirm gate (spec 05) stays the only way out.

## QA Plan

### Features

- F1 — `POST /api/presence/drafts` (new): the planner (Anthropic client, mocked in tests per the `tests/unit/api-clusters-label.test.ts` pattern) receives the accepted strategy (pillars, icpAccounts, cadence) plus — when present — repo facts (spec 03), the insights digest (spec 07), the performance ledger (spec 06), and recent ICP posts; it returns drafts `{ kind: 'post'|'reply', text, pillar, targetUrl?, receipt: { kind: 'repo-fact'|'signal-post', ref } }`
...
- F2 — Prompt grounding: the outgoing planner prompt contains the strategy pillars block always, the facts block / insights block / ledger block exactly when their sources exist, and omits each cleanly when absent
...
- F3 — Accepted drafts land in the spec-05 queue: each generated draft becomes a `publishDraft` row (status `draft`) with pillar and, for replies, `targetUrl`; a `lapId` stamped on the batch makes re-running idempotent
...
- [ ] Prompt contains pillars always; facts/insights/ledger blocks appear iff their sources exist (prompt-capture test ids)
...
- Related docs: `docs/DESIGN-SOCIAL-CANVAS.md`, `lib/publish/x-intent.ts`, `lib/publish/draft-store.ts`, spec 04 (strategy shape), spec 07 (insights digest shape)
- External: none
