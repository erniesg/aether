...
describe('buildCompositionGuidance', () => {
  ...
  it('describes the story top+bottom bands in the prompt suffix', () => {
    const g = buildCompositionGuidance({ preset: 'story' });
    expect(g.promptSuffix.toLowerCase()).toContain('top 14%');
    expect(g.promptSuffix.toLowerCase()).toContain('bottom 20%');
    expect(g.promptSuffix.toLowerCase()).toMatch(/keep|leave|clear/);
  });
  ...
  it('describes the linkedin right+bottom chrome', () => {
    const g = buildCompositionGuidance({ preset: 'linkedin-landscape' });
    expect(g.promptSuffix.toLowerCase()).toMatch(/right edge|right 12%/);
    expect(g.promptSuffix.toLowerCase()).toMatch(/bottom 12%/);
