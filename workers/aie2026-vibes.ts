...
@media(max-width:900px){.shell{display:block;padding:14px}.side{position:static;margin-bottom:14px}h1{font-size:34px}.metrics,.synthesis{grid-template-columns:repeat(2,1fr)}.grid{grid-template-columns:1fr}.wall{grid-template-columns:repeat(3,1fr);grid-auto-rows:100px}.atlas-map{height:560px}.atlas-node{width:150px!important}.atlas-pairs{grid-template-columns:1fr}.cluster-media{grid-template-columns:repeat(3,1fr);grid-auto-rows:92px}}
