...
const DATA_VERSION = 'root777-context175-voice-names-1779215900';
...
const voiceName=(v)=>{const n=String(v.name||'').replace(/\\s+/g,' ').trim();if(n&&!n.includes('#')&&!/^\\d+(\\.\\d+)?\\s+(comments?|likes?|reactions?)$/i.test(n)&&n.length<96)return n;return handleDisplay(v.handle,v.platform)};
