...
@media(max-width:900px){.shell{display:block;padding:14px}.side{position:static;margin-bottom:14px}h1{font-size:34px}.metrics{grid-template-columns:repeat(2,1fr)}.grid{grid-template-columns:1fr}.wall{grid-template-columns:repeat(3,1fr);grid-auto-rows:100px}.atlas-map{height:560px}.atlas-node{width:150px!important}.atlas-pairs{grid-template-columns:1fr}}
</style>
</head>
<body>
<div class="shell">
...
<script>
const state={data:null,tab:'clusters',mediaPage:0,query:'',selectedTheme:null};
...
function renderAtlas(){const d=state.data;const atlas=buildAtlasLayout(d.themes||[]);const cov=clusterCoverage();const lines=atlas.links.map(link=>'<line x1="'+(link.source.x*100).toFixed(2)+'" y1="'+(link.source.y*100).toFixed(2)+'" x2="'+(link.target.x*100).toFixed(2)+'" y2="'+(link.target.y*100).toFixed(2)+'" stroke="#de7340" stroke-width="'+Math.max(.35,link.similarity*2.5).toFixed(2)+'" opacity="'+Math.min(.72,.16+link.similarity).toFixed(2)+'"/>').join('');const nodes=atlas.nodes.map(node=>'<button class="atlas-node '+(state.selectedTheme===node.themeId?'selected':'')+'" data-theme-node="'+escapeHtml(node.themeId)+'" style="left:'+(node.x*100).toFixed(2)+'%;top:'+(node.y*100).toFixed(2)+'%;width:'+node.width+'px"><h4><span class="atlas-dot"></span>'+escapeHtml(node.label)+'</h4><span class="meta">'+node.count+' refs</span></button>').join('');return '<section class="atlas" data-testid="cluster-distance-map"><div class="atlas-head"><div><p class="eyebrow">cluster atlas</p><h2>How the event stories sit together</h2><p>Nearby nodes share recurring language, authors, and cited refs. Farther nodes are separate pockets of the public recap.</p></div><span class="chip">'+fmt(cov.rootRefs)+' root refs mapped</span></div><div class="atlas-map"><svg class="atlas-links" viewBox="0 0 100 100" preserveAspectRatio="none">'+lines+'</svg>'+nodes+'</div><p class="atlas-note">Distance is computed from cluster labels, summaries, keywords, and up to 90 refs per cluster using local term vectors. It is a readable map of evidence similarity, not an embedding model or sentiment score.</p><div class="atlas-pairs"><div class="atlas-pair"><span class="meta">shared signals</span>'+atlasPairList(atlas.shared)+'</div><div class="atlas-pair"><span class="meta">separate pockets</span>'+atlasPairList(atlas.separate)+'</div></div></section>'}
function bindAtlas(){document.querySelectorAll('[data-theme-node]').forEach(node=>{node.onclick=()=>{const id=node.getAttribute('data-theme-node');state.selectedTheme=state.selectedTheme===id?null:id;renderContent()}})}
function renderClusterCard(t,i){const d=state.data;const posts=(t.postIds||[]).map(id=>d.postsById[id]).filter(Boolean).sort((a,b)=>(b.reachScore||0)-(a.reachScore||0));const rootCount=(t.rootPostIds||t.postIds||[]).length;const attachedCount=(t.attachedPostIds||[]).length;const media=posts.flatMap(p=>(p.media||[]).map(m=>({...m,postUrl:p.url}))).filter(isImageMedia).slice(0,4);const selected=state.selectedTheme===t.themeId;return '<div class="card '+(selected?'selected':i<2?'hot':'')+'" data-cluster-card="'+escapeHtml(t.themeId)+'"><div class="row"><h3>'+escapeHtml(t.label)+'</h3><span class="chip">'+posts.length+' refs</span><span class="chip">'+rootCount+' roots</span><span class="chip">'+attachedCount+' context</span></div><div class="tags">'+platformMix(t.postIds||[])+'</div><p>'+escapeHtml(t.summary||'')+'</p><div class="post-media">'+media.map(m=>'<a href="'+m.postUrl+'" target="_blank"><img src="'+mediaUrl(m)+'" loading="lazy" alt=""></a>').join('')+'</div><div class="snips">'+posts.slice(0,3).map(p=>'<p class="snip">['+p.platform+'] '+escapeHtml((p.authorHandle||p.authorName||'unknown')+': '+decodeText(p.text).slice(0,180))+'...</p>').join('')+'</div></div>'}
...
$('visibleCount').textContent=d.themes.length+' clusters · '+fmt(clusterCoverage().clusteredRefs)+'/'+fmt(d.posts.length)+' refs assigned';const themes=[...(d.themes||[])].sort((a,b)=>state.selectedTheme?(a.themeId===state.selectedTheme?-1:b.themeId===state.selectedTheme?1:0):0);$('content').innerHTML=renderAtlas()+'<div class="grid">'+themes.map(renderClusterCard).join('')+'</div>';bindAtlas()}
