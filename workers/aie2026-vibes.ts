...
const DATA_VERSION = 'root757-context174-filter-generic-ai-1779214026';
...
.coverage{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:0;margin-top:14px;border-top:1px solid var(--line);border-bottom:1px solid var(--line)}.coverage-item{padding:10px 12px;border-left:1px solid var(--line);background:#fff}.coverage-item:first-child{border-left:0}.coverage-item b{display:block;font:14px ui-monospace,SFMono-Regular,Menlo,monospace;color:#514b45}.coverage-item span{display:block;margin-top:2px;font:11px ui-monospace,SFMono-Regular,Menlo,monospace;text-transform:uppercase;letter-spacing:.08em;color:var(--dim)}
...
@media(max-width:900px){.shell{display:block;padding:14px}.side{position:static;max-height:none;margin-bottom:14px}h1{font-size:34px;overflow-wrap:normal}.metrics,.synthesis{grid-template-columns:repeat(2,1fr)}.coverage{grid-template-columns:repeat(3,1fr)}.coverage-item:nth-child(4){border-left:0}.grid{grid-template-columns:1fr}.wall{grid-template-columns:repeat(3,1fr);grid-auto-rows:100px}.atlas-map{height:560px}.atlas-node{width:150px!important}.atlas-pairs{grid-template-columns:1fr}.cluster-media{grid-template-columns:repeat(3,1fr);grid-auto-rows:92px}}
  ...
  <aside class="side">
    ...
    <div class="method">
      <details>
        ...
        <p class="method-line"><b>Seeded snowball</b><span>Event terms + source links, expanded through public refs.</span></p>
        <p class="method-line"><b>Corpus</b><span>Recap evidence, not representative sentiment.</span></p>
        <p class="method-line"><b>Reach</b><span>X/YouTube views; LinkedIn engagement only.</span></p>
...
const imageUrl=(m)=>m.path&&isImageUrl(m.path)?localMediaUrl(m):isImageUrl(m.previewUrl)?m.previewUrl:isImageUrl(m.url)?m.url:'';
...
function mediaElement(m){const src=mediaUrl(m);if(!src)return'';return isVideoMedia(m)?'<video src="'+escapeHtml(src)+'" muted playsinline preload="metadata"></video>':'<img src="'+escapeHtml(src)+'" loading="lazy" alt="">'}
...
const cleanVoiceName=(name)=>{let n=String(name||'').replace(/\\s+/g,' ').trim().replace(/\\s+(?:is\\s+)?at\\s+(?:the\\s+)?ai\\s+engineer\\s+singapore.*$/i,'').replace(/\\s+(?:(?:is|was)\\s+)?at\\s+(?:the\\s+)?(?:ai\\s+eng\\s+sg|ai\\s+engineer\\s+sg|ai\\s+engineer\\s+summit|aie\\s+sg).*$/i,'').replace(/\\s+@\\s+ai\\s+engineer\\s+singapore.*$/i,'').replace(/\\s+@\\s*(?:ai\\s*(?:eng|engineer)\\s*(?:singapore|sg)|aie\\s*sg|aie)\\b.*$/i,'').replace(/\\s*[-–—]\\s*.*\\b(?:ai\\s+builder\\s+@\\s+aie|aie\\s+sg|ai\\s+eng(?:ineer)?\\s+singapore)\\b.*$/i,'').replace(/\\s*[-–—]\\s*ai\\s+builder\\s*$/i,'').replace(/\\s*\\([^)]*\\b(?:aie\\s+sg|ai\\s+engineer)\\b[^)]*\\)\\s*$/i,'').replace(/\\s+at\\s+aie\\s+singapore.*$/i,'').trim();if(/^[a-z]+(?:\\s+[a-z]+)*$/.test(n))n=n.split(/\\s+/).map(w=>w[0].toUpperCase()+w.slice(1)).join(' ');if(!n||n.includes('#')||/^\\d+(\\.\\d+)?\\s+(comments?|likes?|reactions?)$/i.test(n)||/\\s+·\\s+(luma|events calendar)$/i.test(n)||/^ai engineer singapore\\b/i.test(n)||/\\b(workshop|hackathon|meetup|conference|event|happy hour)\\b.*\\bai engineer singapore\\b/i.test(n)||/\\bcome and join us\\b.*\\bai engineer\\b/i.test(n)||/\\bai engineer day singapore\\b/i.test(n)||/^(i'?ll|i am|here at|hehe i am)\\b.*\\b(ai engineer|aie)\\b/i.test(n)||n.length>=96)return'';return n};
...
function mediaStrip(p){const imgs=(p.media||[]).filter(isImageMedia);return imgs.length?'<div class="post-media">'+imgs.map((m,i)=>'<a href="'+mediaUrl(m)+'" target="_blank" rel="noreferrer" title="media '+(i+1)+' of '+imgs.length+'">'+mediaElement(m)+'</a>').join('')+'</div>':''}
...
function mergeDisplayThemes(themes){const mergeIds=new Set(['atlas-01-minister-balakrishnan-vivian-balakrishnan','atlas-02-built-tools-minister']);const parts=themes.filter(t=>mergeIds.has(t.themeId));if(parts.length<2)return themes;const merged={...parts[0],themeId:'story-vivian-builder-keynote',label:'Vivian builder keynote',summary:'Vivian Balakrishnan was one public story: the viral keynote line, the hands-on second-brain/NanoClaw workflow, and the broader claim that governance should come from people who have actually built with the tools.',postIds:uniqList(...parts.map(t=>t.postIds||[])),rootPostIds:uniqList(...parts.map(t=>t.rootPostIds||[])),attachedPostIds:uniqList(...parts.map(t=>t.attachedPostIds||[])),keywords:uniqList(...parts.map(t=>t.keywords||[])).slice(0,22),mergedThemeIds:parts.map(t=>t.themeId)};return themes.flatMap(t=>t.themeId===parts[0].themeId?[merged]:mergeIds.has(t.themeId)?[]:[t])}
...
function buildAtlasLayout(themes){const vectors=buildClusterVectors(themes);const maxRefs=Math.max(1,...themes.map(t=>(t.postIds||[]).length));const nodes=themes.map((t,index)=>{const angle=(index/Math.max(1,themes.length))*Math.PI*2-Math.PI/2;const radius=0.35;const width=136+Math.round(((t.postIds||[]).length/maxRefs)*34);const lines=Math.ceil(String(t.label||'').length/17);return{themeId:t.themeId,label:t.label,count:(t.postIds||[]).length,x:0.5+Math.cos(angle)*radius,y:0.5+Math.sin(angle)*radius,width,height:48+lines*16,rx:(width/1160)/2,ry:((48+lines*16)/720)/2,mix:platformCounts(t.postIds||[]),index}});const pairs=[];for(let left=0;left<nodes.length;left++){for(let right=left+1;right<nodes.length;right++){pairs.push({source:nodes[left],target:nodes[right],similarity:vectorSimilarity(vectors[left],vectors[right])})}}function keepInside(node){node.x=clamp(node.x,node.rx+0.04,1-node.rx-0.04);node.y=clamp(node.y,node.ry+0.04,1-node.ry-0.04)}for(let step=0;step<520;step++){for(const pair of pairs){const dx=pair.target.x-pair.source.x;const dy=pair.target.y-pair.source.y;const dist=Math.max(0.001,Math.sqrt(dx*dx+dy*dy));const target=0.22+(1-pair.similarity)*0.66;const force=(dist-target)*0.014;const mx=(dx/dist)*force;const my=(dy/dist)*force;pair.source.x+=mx;pair.source.y+=my;pair.target.x-=mx;pair.target.y-=my}for(let left=0;left<nodes.length;left++){for(let right=left+1;right<nodes.length;right++){const a=nodes[left];const b=nodes[right];const dx=b.x-a.x;const dy=b.y-a.y;const overlapX=a.rx+b.rx+0.018-Math.abs(dx);const overlapY=a.ry+b.ry+0.022-Math.abs(dy);if(overlapX>0&&overlapY>0){if(overlapX<overlapY){const push=(overlapX/2)*(dx<0?-1:1);a.x-=push;b.x+=push}else{const push=(overlapY/2)*(dy<0?-1:1);a.y-=push;b.y+=push}}}}for(const node of nodes){node.x+=(0.5-node.x)*0.002;node.y+=(0.5-node.y)*0.002;keepInside(node)}}const ranked=[...pairs].sort((a,b)=>b.similarity-a.similarity);return{nodes,links:ranked.filter(p=>p.similarity>0.035).slice(0,Math.min(14,Math.max(6,themes.length+3))),shared:ranked.slice(0,4),separate:[...ranked].reverse().slice(0,4)}}
...
function renderClusterDetail(t){if(!t)return'';const posts=themePosts(t);const q=state.query.toLowerCase();const shown=posts.filter(p=>!q||[p.text,p.authorName,p.authorHandle,p.platform].join(' ').toLowerCase().includes(q));const rootCount=(t.rootPostIds||t.postIds||[]).length;const attachedCount=(t.attachedPostIds||[]).length;const media=themeMedia(posts,18);return '<section class="cluster-detail" data-testid="cluster-detail"><div class="cluster-actions"><div><p class="eyebrow">cluster detail</p><h2>'+escapeHtml(t.label)+'</h2></div><button class="ghost" data-clear-theme>back to all clusters</button></div><div class="tags">'+platformMix(t.postIds||[])+'<span class="chip">'+posts.length+' refs</span><span class="chip">'+rootCount+' roots</span><span class="chip">'+attachedCount+' context</span></div><p class="lede">'+escapeHtml(t.summary||'')+'</p>'+(media.length?'<div class="cluster-media">'+media.map(m=>'<a href="'+m.postUrl+'" target="_blank" rel="noreferrer">'+mediaElement(m)+'</a>').join('')+'</div>':'')+'<div class="cluster-posts">'+shown.map(postLine).join('')+'</div></section>'}
function renderClusterCard(t,i){const posts=themePosts(t);const rootCount=(t.rootPostIds||t.postIds||[]).length;const attachedCount=(t.attachedPostIds||[]).length;const media=themeMedia(posts,4);const selected=state.selectedTheme===t.themeId;return '<div class="card '+(selected?'selected':i<2?'hot':'')+'" data-cluster-card="'+escapeHtml(t.themeId)+'"><div class="row"><h3>'+escapeHtml(t.label)+'</h3><span class="chip">'+posts.length+' refs</span><span class="chip">'+rootCount+' roots</span><span class="chip">'+attachedCount+' context</span></div><div class="tags">'+platformMix(t.postIds||[])+'</div><p>'+escapeHtml(t.summary||'')+'</p><div class="post-media">'+media.map(m=>'<a href="'+m.postUrl+'" target="_blank">'+mediaElement(m)+'</a>').join('')+'</div><button class="open-cluster" data-cluster-open="'+escapeHtml(t.themeId)+'">explore cluster refs</button><div class="snips">'+posts.slice(0,3).map(p=>'<p class="snip">['+p.platform+'] '+escapeHtml((p.authorHandle||p.authorName||'unknown')+': '+decodeText(p.text).slice(0,180))+'...</p>').join('')+'</div></div>'}
function renderMethodology(){const d=state.data;const cov=clusterCoverage();const m=d.methodology||{};const q=m.querySet||d.querySet||{};const x=q.x||[];const li=q.linkedin||[];const exp=m.expansionQueries||[];const yt=m.youtubeSources||[];const cq=d.clustering;const candidates=(cq?.candidateScores||[]).map(v=>'<li>'+v.clusterCount+' clusters: silhouette '+Number(v.silhouetteScore||0).toFixed(4)+' · inertia '+Number(v.inertia||0).toFixed(4)+' · elbow '+Number(v.elbowScore||0).toFixed(2)+'</li>').join('');const clusterNote=cq?'<details><summary class="meta">cluster notes</summary><ul><li>'+fmt(cov.rootRefs)+' root refs in atlas</li><li>'+fmt(cov.attachedRefs)+' replies/comments attached as context</li><li>'+cq.clusterCount+' raw clusters selected; '+(cq.elbowClusterCount||cq.clusterCount)+' elbow point</li><li>silhouette '+Number(cq.silhouetteScore||0).toFixed(4)+' · inertia '+Number(cq.inertia||0).toFixed(4)+'</li></ul></details><details><summary class="meta">cluster diagnostics</summary><ul>'+candidates+'</ul></details>':'';$('methodDetails').innerHTML='<div class="method-facts"><span>sources '+date(m.sourceDateRange?.start)+' - '+date(m.sourceDateRange?.end)+'</span><span>'+fmt(cov.clusteredRefs)+'/'+fmt(cov.totalRefs)+' refs clustered</span><span>scores normalized within platform</span></div>'+clusterNote+'<details><summary class="meta">query seeds</summary><ul>'+x.concat(exp).slice(0,40).map(v=>'<li><code>'+escapeHtml(v)+'</code></li>').join('')+'</ul></details><details><summary class="meta">youtube sources</summary><ul>'+yt.map(v=>'<li><a href="'+v.url+'" target="_blank" rel="noreferrer">'+escapeHtml(v.title||v.url)+'</a> <span class="meta">'+fmt(v.views)+' views</span></li>').join('')+'</ul></details>'}
...
if(state.tab==='voices'){const voices=d.voices.filter(v=>!state.query||[voiceName(v),v.handle,v.platform].join(' ').toLowerCase().includes(state.query.toLowerCase()));$('visibleCount').textContent=voices.length+' voices';$('content').innerHTML='<div class="grid">'+voices.map(v=>'<div class="card"><div class="row"><h3>'+escapeHtml(voiceName(v))+'</h3><span class="chip">'+v.platform+'</span>'+score(v.reachScore,'voice')+'</div><p class="meta">'+escapeHtml(v.handle||v.profileUrl||'profile')+' · '+v.postCount+' refs · '+publicSignal(v.totalEngagement)+'</p><p><a class="meta" href="'+(v.samplePostUrls?.[0]||v.profileUrl||'#')+'" target="_blank">open sample</a></p></div>').join('')+'</div>';return}
...
fetch('/vibes/aie2026/data?v=${DATA_VERSION}').then(r=>r.json()).then(d=>{d.postsById=Object.fromEntries(d.posts.map(p=>[p.postId,p]));d.rawThemeCount=(d.themes||[]).length;d.themes=mergeDisplayThemes(d.themes||[]);d.media=[];const seen=new Set();let localMediaTotal=0;for(const p of d.posts){for(const m of p.media||[]){if(m.path)localMediaTotal++;const k=mediaKey(m);if(!k||seen.has(k)||!isImageMedia(m))continue;seen.add(k);d.media.push({...m,postUrl:p.url})}}d.media.sort((a,b)=>(b.bytes||0)-(a.bytes||0));d.mediaTotal=d.media.length;d.localMediaTotal=localMediaTotal;state.data=d;render()}).catch(err=>{const message=String(err&&err.message?err.message:err);$('content').innerHTML='<p class="empty">Could not load recap data'+(location.search.includes('debug=1')?': '+escapeHtml(message):'.')+'</p>';console.error(message)})
</script>
</body>
</html>`;
}
