...
const DATA_VERSION = 'root781-context175-media997-auto-wall-1779200716';
...
function startMediaCycle(){if(state.mediaTimer)clearInterval(state.mediaTimer);if(!state.data||mediaPageCount()<2||matchMedia('(prefers-reduced-motion: reduce)').matches)return;state.mediaTimer=setInterval(()=>{const wall=$('wall');if(document.hidden||Date.now()<state.mediaAutoPausedUntil||(wall&&wall.matches(':hover')))return;setMediaPage(state.mediaPage+1)},AUTO_MEDIA_MS)}
