    ...
    steps: [
      {
        ...
        reviewRequired: true,
        autoAdvance: false,
        toolNames: ['motion_start'],
        apiRoutes: ['/api/motion/start'],
        expectedArtifacts: ['grounded brief', 'video plan', 'source receipts'],
        reviewObjectIds: ['source-evidence-0'],
        reviewObjectLabels: ['Tong repo'],
        agentTemplateHints: ['motion-start'],
        editSurfaceLabels: ['script', 'brief', 'proof'],
        verificationLabels: [],
      },
      {
        ...
        reviewRequired: true,
        autoAdvance: false,
        toolNames: ['motion_capture'],
        apiRoutes: ['/api/motion/capture'],
        expectedArtifacts: ['captures', 'cursor targets', 'crop receipts'],
        reviewObjectIds: ['regen-app-frame'],
        reviewObjectLabels: ['Regenerate App frame'],
        agentTemplateHints: ['review-capture'],
        editSurfaceLabels: ['capture', 'recording', 'crop', 'cursor path'],
        verificationLabels: [],
      },
      {
        ...
        reviewRequired: true,
        autoAdvance: false,
        toolNames: ['motion_export_pack'],
        apiRoutes: ['/api/motion/export-pack'],
        expectedArtifacts: ['export pack', 'canvas drop candidates', 'pack manifest'],
        reviewObjectIds: ['export-x-9-16-30s'],
        reviewObjectLabels: ['x 9:16 30s export pack'],
        agentTemplateHints: ['export-pack'],
        editSurfaceLabels: ['export', 'pack manifest'],
        verificationLabels: ['subtitles', 'transcript'],
      },
    ],
  },
};
...
describe('TimelineLens', () => {
  ...
  it('renders a creator-facing preview plan with drafts, edit controls, and engine readiness', () => {
    ...
    expect(screen.getAllByText('Tong repo').length).toBeGreaterThan(0);
    ...
    expect(screen.getAllByText('Regenerate App frame').length).toBeGreaterThan(0);
    expect(screen.getAllByText('x 9:16 30s export pack').length).toBeGreaterThan(0);
    expect(screen.getAllByText('x 9:16 30s').length).toBeGreaterThan(0);
    ...
    expect(screen.getByText('script / brief / proof')).toBeInTheDocument();
    expect(screen.getByText('capture / recording / crop')).toBeInTheDocument();
