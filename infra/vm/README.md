...
Reusable trusted VM setup for `aether`.
...
browser/device flow shown by Claude Code. API-key modes such as `OPENAI_API_KEY`
or `ANTHROPIC_API_KEY` are separate from subscription login and should stay in a
